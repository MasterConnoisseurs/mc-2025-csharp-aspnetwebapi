...
public class GetProvidersTests
{
    ...
    [Fact]
    public async Task GetProviders_ReturnsOk_NoParameters()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetProviders(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((long? id, bool? isActiveOnly, long? mandatoryId, CancellationToken _) =>
            {
                var filteredList = MockData.Reference.Providers.ProvidersList().Where(x => (id == null || x.Id == id) && (isActiveOnly == null || x.IsActive == isActiveOnly)).ToList();
                if (mandatoryId == null) return TransactionResult<IEnumerable<Providers>>.Success(filteredList);
                {
                    var mandatoryItem = MockData.Reference.Providers.ProvidersList().Where(x => x.Id == mandatoryId).ToList().First();
        ...
        // Act
        var result = await _controller.GetProviders(null, null, null);

        // Assert
        var statusCodeResult = Assert.IsType<OkObjectResult>(result);
        var returnData = Assert.IsAssignableFrom<IEnumerable<Providers>>(statusCodeResult.Value);
    ...
    [Fact]
    public async Task GetProviders_ReturnsOk_WithParameters()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetProviders(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
        ...
        // Act
        var result = await _controller.GetProviders(1, null, null);
    ...
    [Fact]
    public async Task GetProviders_ReturnsInternalServerError()
    {
        // Arrange
        SetUserClaims();
        var resultData = TransactionResult<IEnumerable<Providers>>.Failure(new Exception("Service failed"));
        _mockReferenceV1Service.Setup(s => s.GetProviders(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task GetProviders_ReturnsRequestTimeout()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetProviders(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task GetProviders_ReturnsUnauthorized()
    {
        // Arrange
