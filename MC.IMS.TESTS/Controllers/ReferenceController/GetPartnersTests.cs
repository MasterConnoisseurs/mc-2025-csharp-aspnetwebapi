...
public class GetPartnersTests
{
    ...
    [Fact]
    public async Task GetPartners_ReturnsOk_NoParameters()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetPartners(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((long? id, bool? isActiveOnly, long? mandatoryId, CancellationToken _) =>
            {
                var filteredList = MockData.Reference.Partners.PartnersList().Where(x => (id == null || x.Id == id) && (isActiveOnly == null || x.IsActive == isActiveOnly)).ToList();
                if (mandatoryId == null) return TransactionResult<IEnumerable<Partners>>.Success(filteredList);
                {
                    var mandatoryItem = MockData.Reference.Partners.PartnersList().Where(x => x.Id == mandatoryId).ToList().First();
        ...
        // Act
        var result = await _controller.GetPartners(null, null, null);

        // Assert
        var statusCodeResult = Assert.IsType<OkObjectResult>(result);
        var returnData = Assert.IsAssignableFrom<IEnumerable<Partners>>(statusCodeResult.Value);
    ...
    [Fact]
    public async Task GetPartners_ReturnsOk_WithParameters()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetPartners(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
        ...
        // Act
        var result = await _controller.GetPartners(1, null, null);
    ...
    [Fact]
    public async Task GetPartners_ReturnsInternalServerError()
    {
        // Arrange
        SetUserClaims();
        var resultData = TransactionResult<IEnumerable<Partners>>.Failure(new Exception("Service failed"));
        _mockReferenceV1Service.Setup(s => s.GetPartners(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task GetPartners_ReturnsRequestTimeout()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetPartners(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task GetPartners_ReturnsUnauthorized()
    {
        // Arrange
