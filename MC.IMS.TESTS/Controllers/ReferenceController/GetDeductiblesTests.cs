...
public class GetDeductiblesTests
{
    ...
    [Fact]
    public async Task GetDeductibles_ReturnsOk_NoParameters()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetDeductibles(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((long? id, bool? isActiveOnly, long? mandatoryId, CancellationToken _) =>
            {
                var filteredList = MockData.Reference.Deductibles.DeductiblesList().Where(x => (id == null || x.Id == id) && (isActiveOnly == null || x.IsActive == isActiveOnly)).ToList();
                if (mandatoryId == null) return TransactionResult<IEnumerable<Deductibles>>.Success(filteredList);
                {
                    var mandatoryItem = MockData.Reference.Deductibles.DeductiblesList().Where(x => x.Id == mandatoryId).ToList().First();
        ...
        // Act
        var result = await _controller.GetDeductibles(null, null, null);

        // Assert
        var statusCodeResult = Assert.IsType<OkObjectResult>(result);
        var returnData = Assert.IsAssignableFrom<IEnumerable<Deductibles>>(statusCodeResult.Value);
    ...
    [Fact]
    public async Task GetDeductibles_ReturnsOk_WithParameters()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetDeductibles(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
        ...
        // Act
        var result = await _controller.GetDeductibles(1, null, null);
    ...
    [Fact]
    public async Task GetDeductibles_ReturnsInternalServerError()
    {
        // Arrange
        SetUserClaims();
        var resultData = TransactionResult<IEnumerable<Deductibles>>.Failure(new Exception("Service failed"));
        _mockReferenceV1Service.Setup(s => s.GetDeductibles(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task GetDeductibles_ReturnsRequestTimeout()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetDeductibles(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task GetDeductibles_ReturnsUnauthorized()
    {
        // Arrange
