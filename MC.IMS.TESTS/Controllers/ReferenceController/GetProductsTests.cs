...
public class GetProductsTests
{
    ...
    [Fact]
    public async Task GetProducts_ReturnsOk_NoParameters()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetProducts(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((long? id, bool? isActiveOnly, long? productCategoryId, long? mandatoryId, CancellationToken _) =>
            {
                var filteredList = MockData.Reference.Products.ProductsList().Where(x => (id == null || x.Id == id) && (isActiveOnly == null || x.IsActive == isActiveOnly) && (productCategoryId == null || x.ProductCategoryId == productCategoryId)).ToList();
                if (mandatoryId == null) return TransactionResult<IEnumerable<Products>>.Success(filteredList);
                {
                    var mandatoryItem = MockData.Reference.Products.ProductsList().Where(x => x.Id == mandatoryId).ToList().First();
        ...
        var returnData = Assert.IsAssignableFrom<IEnumerable<Products>>(statusCodeResult.Value);
    ...
    [Fact]
    public async Task GetProducts_ReturnsOk_WithParameters()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetProducts(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
        ...
        // Act
        var result = await _controller.GetProducts(1, null, null, null);
    ...
    [Fact]
    public async Task GetProducts_ReturnsInternalServerError()
    {
        // Arrange
        SetUserClaims();
        var resultData = TransactionResult<IEnumerable<Products>>.Failure(new Exception("Service failed"));
        _mockReferenceV1Service.Setup(s => s.GetProducts(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task GetProducts_ReturnsRequestTimeout()
    {
        // Arrange
        SetUserClaims();
        _mockReferenceV1Service.Setup(s => s.GetProducts(It.IsAny<long?>(), It.IsAny<bool?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>()))
