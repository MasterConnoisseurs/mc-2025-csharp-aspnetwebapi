...
public class GetSummaryYearToYearTests
{
    ...
    public GetSummaryYearToYearTests()
    {
        ...
        var controllerContext = new ControllerContext
        {
            ...
            {
                ActionName = "GetSummaryYearToYear"
            }
        };
    ...
    [Fact]
    public async Task GetSummaryYearToYear_ReturnsOk_NoParameters()
    {
        // Arrange
        SetUserClaims();
        _mockDboV1Service.Setup(s => s.GetSummaryYearToYear(It.IsAny<CancellationToken>()))
            .ReturnsAsync((CancellationToken _) =>
            {
                var filteredList = MockData.Dbo.SummaryYearToYear.SummaryYearToYearList().ToList();
                return TransactionResult<IEnumerable<SummaryYearToYear>>.Success(filteredList);
            });

        // Act
        var result = await _controller.GetSummaryYearToYear();

        // Assert
        var statusCodeResult = Assert.IsType<OkObjectResult>(result);
        var returnData = Assert.IsAssignableFrom<IEnumerable<SummaryYearToYear>>(statusCodeResult.Value);
    ...
    [Fact]
    public async Task GetSummaryYearToYear_ReturnsInternalServerError()
    {
        // Arrange
        SetUserClaims();
        var resultData = TransactionResult<IEnumerable<SummaryYearToYear>>.Failure(new Exception("Service failed"));
        _mockDboV1Service.Setup(s => s.GetSummaryYearToYear(It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task GetSummaryYearToYear_ReturnsRequestTimeout()
    {
        // Arrange
        SetUserClaims();
        _mockDboV1Service.Setup(s => s.GetSummaryYearToYear(It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task GetSummaryYearToYear_ReturnsUnauthorized()
    {
        // Arrange
