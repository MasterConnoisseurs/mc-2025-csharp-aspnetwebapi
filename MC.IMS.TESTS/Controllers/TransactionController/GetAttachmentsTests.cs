...
public class GetAttachmentsTests
{
    ...
    [Fact]
    public async Task GetAttachments_ReturnsOk_NoParameters()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetAttachments(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
            .ReturnsAsync((long? id, long? referenceId, long? attachmentTypeId, CancellationToken _, int _, int _) =>
            {
                var filteredList = MockData.Transaction.Attachments.AttachmentsList().Where(x => (id == null || x.Id == id) && (referenceId == null || x.ReferenceId == referenceId) && (attachmentTypeId == null || x.AttachmentTypeId == attachmentTypeId)).ToList();
                return TransactionResult<IEnumerable<Attachments>>.Success(filteredList);
            });

        // Act
        var result = await _controller.GetAttachments(null, null, null);

        // Assert
        var statusCodeResult = Assert.IsType<OkObjectResult>(result);
        var returnData = Assert.IsAssignableFrom<PagedResponse<IEnumerable<Attachments>>>(statusCodeResult.Value);
    ...
    [Fact]
    public async Task GetAttachments_ReturnsOk_WithParameters()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetAttachments(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
        ...
        // Act
        var result = await _controller.GetAttachments(1, null, null);
    ...
    [Fact]
    public async Task GetAttachments_ReturnsInternalServerError()
    {
        // Arrange
        SetUserClaims();
        var resultData = TransactionResult<IEnumerable<Attachments>>.Failure(new Exception("Service failed"));
        _mockTransactionV1Service.Setup(s => s.GetAttachments(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
    ...
    [Fact]
    public async Task GetAttachments_ReturnsRequestTimeout()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetAttachments(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
    ...
    [Fact]
    public async Task GetAttachments_ReturnsUnauthorized()
    {
        // Arrange
