...
public class GetPolicyDeductiblesTests
{
    ...
    public GetPolicyDeductiblesTests()
    {
        ...
        var controllerContext = new ControllerContext
        {
            ...
            {
                ActionName = "GetPolicyDeductibles"
            }
        };
    ...
    [Fact]
    public async Task GetPolicyDeductibles_ReturnsOk_NoParameters()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetPolicyDeductibles(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
            .ReturnsAsync((long? id, long? policyId, long? policyTypeId, CancellationToken _, int _, int _) =>
            {
                var filteredList = MockData.Transaction.PolicyDeductibles.PolicyDeductiblesList().Where(x =>
                    (id == null || x.Id == id) &&
                    (policyId == null || x.PolicyId == policyId) &&
                    (policyTypeId == null || x.PolicyTypeId == policyTypeId)
                    ).ToList();
                return TransactionResult<IEnumerable<PolicyDeductibles>>.Success(filteredList);
            });

        // Act
        var result = await _controller.GetPolicyDeductibles(null, null, null);

        // Assert
        var statusCodeResult = Assert.IsType<OkObjectResult>(result);
        var returnData = Assert.IsAssignableFrom<PagedResponse<IEnumerable<PolicyDeductibles>>>(statusCodeResult.Value);
    ...
    [Fact]
    public async Task GetPolicyDeductibles_ReturnsOk_WithParameters()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetPolicyDeductibles(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
        ...
        // Act
        var result = await _controller.GetPolicyDeductibles(10, null, null);
    ...
    [Fact]
    public async Task GetPolicyDeductibles_ReturnsInternalServerError()
    {
        // Arrange
        SetUserClaims();
        var resultData = TransactionResult<IEnumerable<PolicyDeductibles>>.Failure(new Exception("Service failed"));
        _mockTransactionV1Service.Setup(s => s.GetPolicyDeductibles(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
    ...
    [Fact]
    public async Task GetPolicyDeductibles_ReturnsRequestTimeout()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetPolicyDeductibles(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
    ...
    [Fact]
    public async Task GetPolicyDeductibles_ReturnsUnauthorized()
    {
        // Arrange
