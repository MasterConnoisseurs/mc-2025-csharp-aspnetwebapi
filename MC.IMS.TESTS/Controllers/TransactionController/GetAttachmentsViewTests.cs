...
public class GetAttachmentsViewTests
{
    ...
    public GetAttachmentsViewTests()
    {
    ...
    [Fact]
    public async Task GetAttachmentsView_ReturnsOk_NoParameters()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetAttachmentsView(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
            .ReturnsAsync((long? id, long? referenceId, long? attachmentTypeId, CancellationToken _, int _, int _) =>
            {
                var filteredList = MockData.Transaction.View.AttachmentsView.AttachmentsViewList().Where(x => (id == null || x.Id == id) && (referenceId == null || x.ReferenceId == referenceId) && (attachmentTypeId == null || x.AttachmentTypeId == attachmentTypeId)).ToList();
                return TransactionResult<IEnumerable<AttachmentsView>>.Success(filteredList);
            });

        // Act
        var result = await _controller.GetAttachmentsView(null, null, null);

        // Assert
        var statusCodeResult = Assert.IsType<OkObjectResult>(result);
        var returnData = Assert.IsAssignableFrom<PagedResponse<IEnumerable<AttachmentsView>>>(statusCodeResult.Value);
    ...
    [Fact]
    public async Task GetAttachmentsView_ReturnsOk_WithParameters()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetAttachmentsView(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
        ...
        // Act
        var result = await _controller.GetAttachmentsView(1, null, null);
    ...
    [Fact]
    public async Task GetAttachmentsView_ReturnsInternalServerError()
    {
        // Arrange
        SetUserClaims();
        var resultData = TransactionResult<IEnumerable<AttachmentsView>>.Failure(new Exception("Service failed"));
        _mockTransactionV1Service.Setup(s => s.GetAttachmentsView(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
    ...
    [Fact]
    public async Task GetAttachmentsView_ReturnsRequestTimeout()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetAttachmentsView(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
    ...
    [Fact]
    public async Task GetAttachmentsView_ReturnsUnauthorized()
    {
        // Arrange
