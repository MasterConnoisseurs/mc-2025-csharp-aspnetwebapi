...
public class GetPolicyPaymentsViewTests
{
    ...
    public GetPolicyPaymentsViewTests()
    {
        ...
        var controllerContext = new ControllerContext
        {
            ...
            {
                ActionName = "GetPolicyPaymentsView"
            }
        };
    ...
    [Fact]
    public async Task GetPolicyPaymentsView_ReturnsOk_NoParameters()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetPolicyPaymentsView(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
            .ReturnsAsync((long? id, long? policyId, long? policyTypeId, CancellationToken _, int _, int _) =>
            {
                var filteredList = MockData.Transaction.View.PolicyPaymentsView.PolicyPaymentsViewList().Where(x =>
                    (id == null || x.Id == id) &&
                    (policyId == null || x.PolicyId == policyId) &&
                    (policyTypeId == null || x.PolicyTypeId == policyTypeId)
                    ).ToList();
                return TransactionResult<IEnumerable<PolicyPaymentsView>>.Success(filteredList);
            });

        // Act
        var result = await _controller.GetPolicyPaymentsView(null, null, null);

        // Assert
        var statusCodeResult = Assert.IsType<OkObjectResult>(result);
        var returnData = Assert.IsAssignableFrom<PagedResponse<IEnumerable<PolicyPaymentsView>>>(statusCodeResult.Value);
    ...
    [Fact]
    public async Task GetPolicyPaymentsView_ReturnsOk_WithParameters()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetPolicyPaymentsView(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
        ...
        // Act
        var result = await _controller.GetPolicyPaymentsView(1, null, null);
    ...
    [Fact]
    public async Task GetPolicyPaymentsView_ReturnsInternalServerError()
    {
        // Arrange
        SetUserClaims();
        var resultData = TransactionResult<IEnumerable<PolicyPaymentsView>>.Failure(new Exception("Service failed"));
        _mockTransactionV1Service.Setup(s => s.GetPolicyPaymentsView(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
    ...
    [Fact]
    public async Task GetPolicyPaymentsView_ReturnsRequestTimeout()
    {
        // Arrange
        SetUserClaims();
        _mockTransactionV1Service.Setup(s => s.GetPolicyPaymentsView(It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<long?>(), It.IsAny<CancellationToken>(), It.IsAny<int>(), It.IsAny<int>()))
    ...
    [Fact]
    public async Task GetPolicyPaymentsView_ReturnsUnauthorized()
    {
        // Arrange
